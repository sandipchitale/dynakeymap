...
public class DynaKeyMapAction extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent anActionEvent) {
        ...
        Map<MAP_TYPE, String> mapTypeStringMap = shortcutsTable();

        FileEditorManagerEx fileEditorManager = (FileEditorManagerEx) FileEditorManager.getInstance(Objects.requireNonNull(project));

        LightVirtualFile shortcutsActionIdMarkup = new LightVirtualFile("Action | Shortcuts as of " + DateFormatUtil.formatDateTime(System.currentTimeMillis()),
                FileTypeManager.getInstance().getFileTypeByExtension("markdown"),
                mapTypeStringMap.get(MAP_TYPE.ACTION_ID_SHORTCUTS));
        shortcutsActionIdMarkup.setWritable(false);
        FileEditor[] fileEditors = fileEditorManager.openFile(shortcutsActionIdMarkup, true, true);
        ...
        LightVirtualFile keyXModifiersMarkup = new LightVirtualFile("Key x Modifiers as of " + DateFormatUtil.formatDateTime(System.currentTimeMillis()),
                FileTypeManager.getInstance().getFileTypeByExtension("markdown"),
                mapTypeStringMap.get(MAP_TYPE.KEY_X_MODIFIER));
        keyXModifiersMarkup.setWritable(false);
        fileEditors = fileEditorManager.openFile(keyXModifiersMarkup, true, true);
        for (FileEditor fileEditor : fileEditors) {
            if (fileEditor instanceof TextEditorWithPreview textEditorWithPreview) {
                try {
                    // Reflection to the rescue
    ...
    private static Map<MAP_TYPE, String> shortcutsTable() {
        Map<MAP_TYPE, String> maps = new HashMap<>();
        ...
        maps.put(MAP_TYPE.KEY_X_MODIFIER, stringBuilder.toString());

        stringBuilder = new StringBuilder();
        stringBuilder.append("# Action | Keystrokes View\n");
        ...
        maps.put(MAP_TYPE.ACTION_ID_SHORTCUTS, stringBuilder.toString());
